...
function createHeart() {
    // Array of beautiful Valentine colors
    ...
    // Random settings
    const sizeValue = Math.floor(Math.random() * 20) + 15; // 15px to 35px
    const size = sizeValue + "px";
    const color = colors[Math.floor(Math.random() * colors.length)];
    const left = Math.floor(Math.random() * 100) + "%";
    const duration = Math.random() * 3 + 5 + "s"; // Slower for smoother movement
    ...
    // This creates the heart shape perfectly without needing extra CSS pseudo-elements
...
// Create a new heart every 400ms for a cleaner look
